...
```{r histogram}
ggplot(vol, aes(Volume)) + geom_histogram(binwidth = 1)
```
...
```{r scatter}
ggplot(vol, aes(Trial, Volume)) + geom_point() + geom_smooth(method = lm, se = FALSE) + geom_hline(yintercept = mean(vol$Volume))
```


```{r line}
ggplot(vol, aes(Trial, Volume)) + geom_line() + geom_hline(yintercept = mean(vol$Volume))
```
