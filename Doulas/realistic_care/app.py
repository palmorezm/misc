...
app_ui = ui.page_fluid(
    ...
    ui.row(
        ...
        ui.column(6,
            ...
            ui.input_slider("pbr", "Preterm Birth Rate (%)", 0, 50, 21.16),
            ui.input_slider("r", "Preterm Reduction Rate (%)", 0, 10, 1.6),
            ui.input_slider("mgp", "Preterm Average Gest.", 20, 37, 35.52),
...
def server(input, output, session):
    ...
    @render.text
    def possible_births():
        return(f"{input.n_doulas()*input.bpd()*12} Doulas Supported Births Possible") # births with doula care
    ...
    def preterm_realbirths():
        return(f"{input.n_doulas()*input.bpd()*12*(input.pbr()/100)} Preterm Births with Doulas") # births before 37 weeks
    ...
    def preterm_realavoided():
        ...
        return(f"{round(avoided_preterm, 2)} Births Avoided Preterm")
    ...
    def preterm_gest():
        ...
        return(f"{days} Days Preterm on Average") # answer in days
    ...
    def preterm_realavoidedcost():
        ...
        avoidedcost = avoided*input.cost_per_day()*average_days_preterm
        return(f"{round(avoidedcost, 0)} Costs Avoided from Reduction in Preterm Births")
